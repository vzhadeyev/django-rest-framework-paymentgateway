...
from payment_gateway.models import WalletOneTransaction, Invoice
...
class WalletOneConfirmSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        data = WalletOneTransactionDTO(invoice_id=validated_data['WMI_PAYMENT_NO'],
                                       money_amount=validated_data['WMI_PAYMENT_AMOUNT'], **validated_data)
        self.provider.try_pay(data.invoice_id, data)
